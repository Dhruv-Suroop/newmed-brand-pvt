...
- **Motion** (`v3_motion.js`): calm opacity + settle, decelerating curve, nothing bounces. Two non-obvious traps handled — sections are `display:none` so IntersectionObserver can't observe them (elements are armed on `nm:repaint`), and a fast scroll can jump an element past the viewport without it ever intersecting (a rAF-throttled scroll sweep catches those). Reduced-motion and missing-IO both fail open.
- **`assets/graphics/six-stages-onband.svg`** is a v3-only copy with arrows in Off White `#f4f1ea`; the original's `#efe9e0` arrows are invisible on a sunken band but correct on `index.html`'s page background. **Do not edit the original to fix v3.**
- Verified across all 7 sections: no horizontal overflow, no element left stuck invisible.
