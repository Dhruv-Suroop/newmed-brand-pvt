...
Fills: `coral`, `amber`, `cream`, `white`, `ink`, `maroon`, `sunset` (amber→coral), or any CSS color. `opacity` for the translucent process-diagram treatment. Higher-level uses: `ArrowHighlight` (text highlight), `ArrowButton` (CTA), `ArrowBadge` (eyebrow), `ArrowList` (numbered rows), `StepFlow` (journey).
