> **Updated August 2026 against the live brand manual.**
>
> This pack was regenerated, not merely re-exported. What changed:
> - `--nm-coral` and `--nm-orange` (which held `#ff7822`, not a brand colour)
>   were collapsed onto **`--nm-orange: #FF5122`**. Previously, asking for
>   "orange" produced a colour outside the palette.
> - Cream corrected from `#f8f3ea` to **`#F4F1EA`**.
> - `amber` renamed to **`yellow`** throughout.
> - Tagline updated to **"Training Talent. Deploying Care."**
> - The voice-and-tone guide was removed — that section was deleted from the
>   manual deliberately.
> - Added `engine/` (the production arrow and gradient code), five new
>   guideline documents, and `corpus/` — the full manual offline.
>
> Where this readme and `corpus/` disagree, the corpus wins.
...
The brand strategy is **clinical calm** — the voice of a senior clinician who states facts plainly, never oversells, and is calm because they are competent. The visual world matches: warm cream backgrounds, soft orange/yellow gradient "orbs", a deep-brown ink, and a single confident orange primary.
...
**Overall vibe:** warm, soft, confident, unhurried. Sunrise/orb warmth on cream paper — optimistic but calm, never loud. A single saturated orange does all the shouting so nothing else has to.

**Color** — Orange `#FF5122` is the primary (the symbol, CTAs, key accents). Yellow `#FF9D00` and orange `#FF9D00` extend it warm. Deep brown ink `#391E1A` is the "black" — all primary text, never pure `#000`. Maroon `#7C3134` adds depth; teal `#93CCCD` is the one cool counterpoint (use sparingly). Backgrounds are cream `#F4F1EA` (page) and `#EFE9E0` (sunken); white for raised cards. **Max one or two background tones per surface.**

**The orb / gradient glow motif** — ambient background warmth. **Gradients are always organic, soft, and flowing — never hard-edged circles, never sharp lines.** Layered off-center radial washes fading to transparent, heavily blurred, bleeding off edges (see the master art in `assets/backgrounds/`). Component: `Orb` (tones orange/yellow/mix). Gradient tokens: `--gradient-arrow` (yellow→orange, for arrow fills), `--gradient-sunrise`.
...
**Shadows** — low, warm, diffuse, tinted `rgba(57,30,26,·)`. Orange CTAs may carry a soft orange glow (`--shadow-orange`). Never hard black drop shadows.
